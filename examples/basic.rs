...
fn main() {
    ...
    println!("strain: wb_95={:.3} affected_gt0.15={:.2}", strain.mps_wb_95, strain.affected_volume_frac_gt_015);
    ...
    let cfg = SimConfig { mc_runs: 200, max_days: 300, seed: Some(42) };
    let traces = run_mc_recovery(&dmg, &mods, &cfg);
    let sum = summarize(&traces, strain, dmg);
    println!("MC recovery (N={}): median 80% = {:.0}d  (p05={:.0} p95={:.0})  med setbacks={:.1}",
             sum.n_runs, sum.median_days_80pct, sum.p05_days_80pct, sum.p95_days_80pct, sum.median_setbacks);
